...
- [-] 20. status and foucs items needs to match the color, status and size to the mockup here  mockup/3%20-%20Challenges___Competitions_v7.htm, also only changes color on mouse hover. (Reviewed as not done)
- [-] 21. Open For Applications on top needs a red circle on the left (same mockup as 20.) (Reviewed as not done)
- [-] 22. Tabs under status and focus have a bigger font and bold (mockup as reference, same as 20.) (Reviewed as not done)
...
- [-] 26. Sort dropdown menu is too big and not working (Review: Working but not styled as it should)
- [-] 25.1 For featured card, Market Access text should be ajudsted to match mockup (see 20. mockup) (Review: mockups text is 22px and current is 28px)
- [x] 25.2 MORE COMING Q4 2025 should be configurable text, and the style should be like the mockuo (check 20. mockup)
- [-] 25.3 Lets create some mockup data for future competitions to check style and functionality, look into all possible status and focus and make sure we have data for everything (Review: Some data is still missing, please check the filter and split and add more data including status and focus)
...
- [-] 30. Change the text to white font. Details look into original google docs and mockup. (Review: Startup tab Apply now in non-featured cards have black text and should be white text)
- [-] 31. Like 29, each card has a different focus style and icon. Follow original document and mockup. (Review: Top focus filter is displaying correctly but card icon is not displayed correctly. Should use the same icon and color as the focus )
- [x] 32. Change "MORE COMING Q3 2025" text to "More Coming Q2 2026" and change size (follows the mockup in 20.)
- [-] 34. Footer should be with white background in the internal pages and have similar size and styles for all elements. Some extra details above
...
- [-] 34. Footer content, aligment, elements, text should be exactly like the main page, internal pages should have white background and black text. buttons also respect the "white" style in the internal pages, Everything else is exactly like the home page
