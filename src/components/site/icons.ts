 ...
 *
 * Icon paths are from Lucide (https://lucide.dev), ISC licensed, inlined as
 * raw path markup so the site carries no icon package dependency and makes no
 * CDN request. All icons share the Lucide canvas: 24x24, stroke-based,
 * `fill="none"`, round caps/joins — matching the hand-written stroke SVGs
 * already in the homepage shell.
 *
 * ICON_PATHS is the single source of truth for icon names. Both `Icon.astro`
 * (render) and `src/lib/homepage/schema.ts` (the Keystatic icon select) derive
 * from it, so an icon cannot exist in one and not the other.
 *
 * To add an icon: copy the inner markup of the Lucide SVG (everything between
 * the <svg> tags) and add it here under its Lucide name. That is the whole
 * requirement — it renders and becomes CMS-selectable immediately.
 *
 * ICON_LABELS is an optional presentation-only override for the CMS select.
 * An icon with no label is still offered, labelled by its raw name.
 */
export const ICON_PATHS: Record<string, string> = {
  ...
  rocket:
    '<path d="M4.5 16.5c-1.5 1.26-2 5-2 5s3.74-.5 5-2c.71-.84.7-2.13-.09-2.91a2.18 2.18 0 0 0-2.91 0"/><path d="m12 15-3-3a22 22 0 0 1 2-3.95A12.88 12.88 0 0 1 22 2c0 2.72-.78 7.5-6 11a22.35 22.35 0 0 1-4 2"/><path d="M9 12H4s.55-3.03 2-4c1.62-1.08 5 0 5 0"/><path d="M12 15v5s3.03-.55 4-2c1.08-1.62 0-5 0-5"/>',
  "scroll-text":
...
/** Human-readable labels for the Keystatic icon select, keyed by ICON_PATHS name. */
